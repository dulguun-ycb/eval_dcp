...
int pnet_init_only (pnet_t * net, const pnet_cfg_t * p_cfg)
{
   ...
   /* Fieldbus Application Layer Service Protocol Machine */
   ...
   /* Scheduler Protocol Machine */ 
   /* verantwortlich für die zeitliche Koordination der zyklischen Datenkommunikation */
...
*/
   ...
   /* Provider protocol machine: This handles cyclic sending of data (and IOPS). Initializes transmit buffers.*/
   //pf_ppm_init (net);

   /* Alarm Protocol Machine */
   /* Sends and receives low- and high prio alarm Ethernet frames. */
   //pf_alarm_init (net);
   

   /* incoming cyclic Profinet data Ethernet frames to frame handlers */
   ...
   /* Context Management Protocol Machine Device (CMDEV) */
   /* This handles connection establishment for IO-Devices. */
   //pf_cmdev_exit (net); /* Prepare for re-init */
   //pf_cmdev_init (net);

   /* Context Management RPC Device Protocol Machine (CMRPC) */
   /* This handles RPC communication via UDP; for example connect, release, dcontrol, ccontrol and parameter read and write. */
   //pf_cmrpc_init (net);
